...
рынке, что еще больше повышает стоимость данного оборудования при закупке по параллельному импорту.

![Описание бизнес контекста](./diagrams/business_description.svg "Описание бизнес контекста")

## Бизнес драйверы

- Большое время обработки заявки на тестирование (недели, доходит до месяцев)
...
"характеристик здоровья". Другое програмное обеспечение на тестируемые устройства устанавливать запрещено так как оно 
...
- Должен иметь пользовательский интерфейс для управления тестировщиками.
...
- UC005: Просмотр информации состояния заявок
  - администратор запрашивает информацию о статусе заявки/заявок (SH3);
...
- UC008: Просмотр удаляет поддержку методики тестирования
...
Ключевым элементом бизнес-потока является событие. Обратите внимание, что это не точный процесс, и он используется 
только для разъяснения концепций, задействованных в бизнес-процессах. 
### UC003: Обработка заявки на тестирование
...
В ходе event storming выявленно три основных сущности изменение состояния которых вызывает события: заявка, тест и 
тестовый стенд. Таким образом выделяется 3 агрегата: Трекер заявок отслеживает контекст всех обрабатываемых заявок в 
системе, Обработчик заявок отслеживает контекст выполнения теста выполняемого в рамках выполнения заявки на тестирование 
по определенной методике, Менеджер тестовых стендов отслеживает контекст всех присутствующих тестовых стендов. Также 
имеется Генератор нагрузки который только публикует события об изменении состояния теста.   

На диаграме ниже представленна интеграция контекстов
![context integration](./diagrams/context_integration.svg "context integration")
